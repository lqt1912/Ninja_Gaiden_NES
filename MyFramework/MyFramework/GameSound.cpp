...
Sound::Sound(HWND hWnd)
{
	...
	if (FAILED(result))
	{
		DebugOut((wchar_t*)"Can not create device");
	}
	...
	if (FAILED(result))
	{
		DebugOut((wchar_t*)"Can not create primaryBuffer");
	}
	volume = 100.0f;
	isMute = false;
}
...
void Sound::loadSound(char* fileName, std::string name)
{
	...
	if (error != 0)
	{
		DebugOut((wchar_t*)" Can not load: %s", fileName);
		return;
	}
	...
	{
		DebugOut((wchar_t*)" file format does not support: %s", fileName);
	}
	...
	if (FAILED(result))
	{
		DebugOut((wchar_t*)" Can not create secondaryBuffer ");
		return;
	}
	...
	if (error != 0)
	{
		DebugOut((wchar_t*)" Can not close file ");
	}
